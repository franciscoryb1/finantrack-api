    ...
    if (statements.length > 0) {
      ...
      for (const inst of allInstallments) {
        ...
        const purchaseReceivedCents = inst.purchase.sharedAmountCents
          ? inst.purchase.sharedReimbursements.reduce(
        ...
        installmentItems.push({
          ...
          installmentInfo: {
            ...
            reimbursementAmountCents:
              inst.purchase.reimbursementAmountCents ?? null,
            reimbursementAccountId:
              inst.purchase.reimbursementAccountId ?? null,
            reimbursementAt:
              inst.purchase.reimbursementAt?.toISOString() ?? null,
      ...
      return {
        ...
        type:
          m.type === MovementType.INCOME
            ? 'INCOME'
            : m.type === MovementType.STATEMENT_PAYMENT
              ? 'STATEMENT_PAYMENT'
              : m.type === MovementType.TRANSFER_OUT
                ? 'TRANSFER_OUT'
                : m.type === MovementType.TRANSFER_IN
                  ? 'TRANSFER_IN'
                  : m.type === MovementType.BALANCE_ADJUSTMENT
                    ? 'BALANCE_ADJUSTMENT'
                    : 'EXPENSE',
        ...
        transferData: m.transferOut
          ? {
              ...m.transferOut,
              transferredAt: m.transferOut.transferredAt.toISOString(),
            }
          : m.transferIn
            ? {
                ...m.transferIn,
                transferredAt: m.transferIn.transferredAt.toISOString(),
              }
            : null,
        ...
        incomeSource: m.reimbursedPurchase
          ...
          : m.sharedFromMovementId || m.sharedFromCreditCardPurchaseId
            ...
            : null,
        balanceAdjustmentIncreased:
          m.type === MovementType.BALANCE_ADJUSTMENT ? m.amountCents > 0 : null,
      };
    });
