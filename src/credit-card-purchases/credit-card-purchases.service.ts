...
export class CreditCardPurchasesService {
  ...
  private async createReimbursementMovement(
    ...
    if (!account)
      throw new BadRequestException(
        'Reimbursement account not found or inactive',
      );
  ...
  /**
   * Valida que el consumo entre dentro del límite disponible de la tarjeta
   * (suma de cuotas PENDING/BILLED + el nuevo total <= límite).
   */
  private async assertWithinCardLimit(
    ...
    if (committedCents + addCents > limitCents) {
      throw new BadRequestException('Credit card limit exceeded');
    }
  }

  /**
   * Determina el sequenceNumber del statement al que pertenece la primera cuota
   * de un consumo según su fecha, creando los statements que falten. Replica el
   * comportamiento de tarjetas en Argentina (corte por fecha de cierre).
   */
  private async resolveFirstStatementSequence(
    tx: Prisma.TransactionClient,
    userId: number,
    creditCardId: number,
    occurredDate: Date,
    ...
    if (openStatement) {
      return openStatement.sequenceNumber;
    }
    ...
    return stmt.sequenceNumber;
  }

  /** Genera las cuotas PENDING de un consumo (sin asignar statement todavía). */
  ...
  async create(userId: number, dto: CreatePurchaseDto) {
    ...
    if (
      sharedAmountCents !== undefined &&
      sharedAmountCents > totalAmountCents
    ) {
      throw new BadRequestException(
        'sharedAmountCents cannot exceed totalAmountCents',
      );
    }
    ...
    const hasReimbursement = !!(
      reimbursementAmountCents && reimbursementAccountId
    );
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      // determinar a qué statement pertenece la primera cuota
      const firstStatementSequence = await this.resolveFirstStatementSequence(
        tx,
        userId,
        creditCardId,
        occurredDate,
      ...
      const purchase = await tx.creditCardPurchase.create({
        data: {
          ...
          ...(hasReimbursement && {
            reimbursementAmountCents,
            reimbursementAccountId,
            reimbursementAt: reimbursementAt
      ...
      if (sharedAmountCents && sharedReimbursementAccountId) {
        ...
        const reimbBalance =
          reimbAccount.currentBalanceCents + sharedAmountCents;
  ...
  /**
   * Ingesta un consumo de tarjeta detectado en un email bancario (vía n8n).
   * Se guarda como DRAFT pendiente de revisión: NO genera cuotas, NO asigna
   * statement y NO valida límite. Toda esa lógica corre recién al confirmar.
   */
  async createFromEmail(dto: CreateFromEmailDto) {
    const {
      userId,
      emailMessageId,
      cardLast4,
    ...
    const existing = await this.prisma.creditCardPurchase.findUnique({
      where: { emailMessageId },
      select: { id: true },
    });
    if (existing) {
      throw new ConflictException('Purchase already exists for this email');
    }

    // 3. Resolver tarjeta por últimos 4 dígitos (sólo si el email los trajo;
    //    puede no haber match → queda sin tarjeta y se asigna al confirmar).
    ...
    // 4. Validar la categoría sugerida por n8n de forma no-bloqueante:
    //    si no es una categoría EXPENSE válida del usuario (o global), se ignora.
    ...
    // 5. Crear el draft pendiente (sin cuotas, sin statement, sin chequeo de límite)
    return this.prisma.creditCardPurchase.create({
      data: {
        userId,
        ...
        description,
        firstStatementSequence: null,
        status: PurchaseStatus.PENDING_REVIEW,
        source: PurchaseSource.EMAIL,
        emailMessageId,
        parserConfidence: parserConfidence ?? null,
        rawEmailData: dto as unknown as Prisma.InputJsonValue,
      },
    });
  }

  /** Cuenta los consumos pendientes de revisión del usuario (para el badge de la nav). */
  async countPending(userId: number): Promise<{ count: number }> {
    const count = await this.prisma.creditCardPurchase.count({
      where: {
        userId,
        isDeleted: false,
        status: PurchaseStatus.PENDING_REVIEW,
  ...
  /** Lista los consumos pendientes de revisión (ingresados por email) del usuario. */
  ...
  /**
   * Confirma un consumo pendiente (con posibles ediciones del usuario) y lo
   * materializa: valida límite, resuelve statement y genera cuotas. A partir de
   * acá es una compra normal (CONFIRMED).
   */
  async confirm(userId: number, purchaseId: number, dto: ConfirmPurchaseDto) {
    const draft = await this.prisma.creditCardPurchase.findFirst({
    ...
    if (draft.status !== PurchaseStatus.PENDING_REVIEW) {
      throw new BadRequestException('Only pending purchases can be confirmed');
    }

    // Combinar el draft con las ediciones del usuario
    const creditCardId = dto.creditCardId ?? draft.creditCardId;
    if (!creditCardId) {
      throw new BadRequestException('A credit card is required to confirm');
    }

    const totalAmountCents = dto.totalAmountCents ?? draft.totalAmountCents;
    const installmentsCount = dto.installmentsCount ?? draft.installmentsCount;
    const occurredDate = dto.occurredAt
      ? new Date(dto.occurredAt)
      : draft.occurredAt;
    const description =
      dto.description !== undefined ? dto.description : draft.description;
    const categoryId =
      dto.categoryId !== undefined ? dto.categoryId : draft.categoryId;
  ...
  /** Descarta un consumo pendiente (no genera cuotas ni statement). */
  async discard(userId: number, purchaseId: number) {
    const draft = await this.prisma.creditCardPurchase.findFirst({
      where: { id: purchaseId, userId },
    ...
    if (draft.status !== PurchaseStatus.PENDING_REVIEW) {
      throw new BadRequestException('Only pending purchases can be discarded');
    }

    return this.prisma.creditCardPurchase.update({
      where: { id: purchaseId },
      data: { status: PurchaseStatus.DISCARDED },
    });
  }
  ...
  async importLegacy(userId: number, dto: ImportLegacyPurchaseDto) {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (
        (committed._sum.amountCents ?? 0) + pendingAmountCents >
        creditCard.limitCents
      ) {
        throw new BadRequestException('Credit card limit exceeded');
      }
      ...
      const firstStatementSequence = statementByKey.get(
        `${fy}-${fm}`,
      )!.sequenceNumber;
  ...
  async createCredit(userId: number, dto: CreateCreditCardCreditDto) {
    const { creditCardId, amountCents, occurredAt, description, categoryId } =
      dto;
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      let stmt =
        existingStmt && !(existingStmt.closingDate < occurredDate)
          ? existingStmt
          : await tx.creditCardStatement.findUnique({
              where: { creditCardId_year_month: { creditCardId, year, month } },
      ...
      await tx.creditCardInstallment.create({
        data: {
          ...
          month,
          status:
            stmt.status === CreditCardStatementStatus.OPEN
              ? CreditCardInstallmentStatus.PENDING
  ...
  async update(userId: number, purchaseId: number, dto: UpdatePurchaseDto) {
    ...
    // Los consumos pendientes de revisión (ingresados por email) se editan/confirman
    // por su propio flujo (confirm/discard), no por este update genérico.
    if (purchase.status !== PurchaseStatus.CONFIRMED) {
      throw new BadRequestException(
        'Pending purchases must be confirmed or discarded',
      );
    }
    ...
    if (sharedAmountCents !== undefined && sharedAmountCents !== null) {
      const effectiveTotal =
        purchaseUpdateData.totalAmountCents ?? purchase.totalAmountCents;
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      const updated = await tx.creditCardPurchase.update({
        ...
        data: {
          ...purchaseUpdateData,
          ...(purchaseUpdateData.occurredAt && {
            occurredAt: new Date(purchaseUpdateData.occurredAt),
          }),
      ...
      if (isChangingReimbursement) {
        ...
        if (clearing) {
          ...
          if (purchase.reimbursementMovementId) {
            await this.deleteReimbursementMovement(
              tx,
              purchase.reimbursementMovementId,
            );
          }
          await tx.creditCardPurchase.update({
            where: { id: purchaseId },
          ...
          const newAmountCents =
            reimbursementAmountCents ?? purchase.reimbursementAmountCents;
          const newAccountId =
            reimbursementAccountId ?? purchase.reimbursementAccountId;
  ...
  async reassignCard(userId: number, purchaseId: number, dto: ReassignCardDto) {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (
        (committed._sum.amountCents ?? 0) + purchase.totalAmountCents >
        newCard.limitCents
      ) {
        throw new BadRequestException('Credit card limit exceeded');
      }
  ...
  async registerSharedReimbursement(
    userId: number,
    purchaseId: number,
    dto: RegisterReimbursementDto,
  ) {
    return this.prisma.$transaction(async (tx) => {
      ...
      if (!purchase.sharedAmountCents)
        throw new BadRequestException('Purchase is not a shared expense');

      const alreadyReceived = purchase.sharedReimbursements.reduce(
        (s, r) => s + r.amountCents,
        0,
      );
      ...
      if (dto.amountCents > pending) {
        throw new BadRequestException(
          `Amount exceeds pending reimbursement (${pending} cents)`,
        );
      }
        ...
        data: {
          ...
          description:
            ...
            (purchase.description
              ? `Reintegro - ${purchase.description}`
              : 'Reintegro'),
          balanceSnapshotCents: newBalance,
          sharedFromCreditCardPurchaseId: purchaseId,
        },
