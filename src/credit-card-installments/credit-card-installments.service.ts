...
export class InstallmentsService {
  ...
  async getOverview(userId: number): Promise<InstallmentsOverviewResponseDto> {
    ...
    const currentPeriodStatements =
      await this.prisma.creditCardStatement.findMany({
        where: {
          userId,
          creditCardId: { in: cardIds },
          year: currentYear,
    ...
    const currentPeriodMap = new Map<
      number,
      (typeof currentPeriodStatements)[number]
    >();
    currentPeriodStatements.forEach((st) =>
      currentPeriodMap.set(st.creditCardId, st),
    );
  ...
  async getCardPeriodDetail(
    ...
    for (const inst of allInstallments) {
      ...
      if (!purchasesMap.has(inst.purchaseId)) {
        ...
        const purchaseReceivedCents = (
          inst.purchase.sharedReimbursements ?? []
        ).reduce(
        ...
        purchasesMap.set(inst.purchaseId, {
          ...
          installmentsRemaining: inst.purchase.installmentsCount - paidCount,
          ...
          sharedExpense: inst.purchase.sharedAmountCents
            ? {
                ...
                pendingAmountCents:
                  inst.purchase.sharedAmountCents - purchaseReceivedCents,
              }
            : null,
          category: inst.purchase.category
            ? {
                ...
                parent: inst.purchase.category.parent
                  ? {
                      id: inst.purchase.category.parent.id,
                      name: inst.purchase.category.parent.name,
                      color: inst.purchase.category.parent.color ?? null,
                    }
                  : null,
              }
            : null,
    ...
    return {
      ...
      payment: payment
        ? {
            ...
            account: {
              id: payment.paidFromAccount.id,
              name: payment.paidFromAccount.name,
            },
          }
        : null,
